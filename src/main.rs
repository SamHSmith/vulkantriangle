...
fn main() {
    let display = Display::connect_to_env().expect("Oops no wayland.");

    let mut devents = display.create_event_queue();

    let gm = GlobalManager::new(&display.attach(devents.token()));

    roundtrip(&mut devents);
    ...
    let compositor = gm
        ...
        .unwrap();
    let waysurface = compositor.create_surface();

    let xdgbase = gm.instantiate_exact::<xdg_wm_base::XdgWmBase>(2).unwrap();

    let xdgsurface = xdgbase.get_xdg_surface(&waysurface);
    let xdgtoplevel = xdgsurface.get_toplevel();
    ...
    let surface = unsafe {vulkano::swapchain::Surface::from_wayland(
        instance.clone(),
        display.c_ptr(),
        waysurface.as_ref().c_ptr(),
        (),
    )
                          .unwrap()};
...
fn roundtrip(devents: &mut EventQueue) {
    println!("doing a roundtrip!");
    devents.sync_roundtrip(&mut (), |_, _, _| {}).unwrap();
}
